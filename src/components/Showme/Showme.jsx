...
const Showme = () => {
  ...
  return (
    <ShowmeContainer>
      <CajaTexto>
        <Titulo>{title}</Titulo>
        <CategoriaLink to={`/categoria/${category}`} categoriaColor={categoriaActual?.color}>
            {category} 
        </CategoriaLink>
      ...
      <ContenedorYT>
        <Link to={`/video/${id}`}>
          <ImagenVideo
            src={linkImg}
            alt={title}
            categoriaColor={categoriaActual?.color}
          />
        </Link>
      </ContenedorYT>
    </ShowmeContainer>
...
const CategoriaLink = styled(Link)`
  ...
  justify-content: center;
  margin: auto;
  background: ${(props) => props.categoriaColor};
  transition: background ease-out 0.5s;
  &:hover {
    background: ${(props) => props.categoriaColor && hexToRgba(props.categoriaColor, 0.85)};
  }
  @media screen and (min-width: 1024px) and (max-width: 1439px) {
    font-size: 1.7rem;
    padding: 0.4rem 0.8rem;
...
const ImagenVideo = styled.img`
  width: 100%;
  border: 3px solid ${(props) => props.categoriaColor};
`;
